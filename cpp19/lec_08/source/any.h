...
class any {
 public:
  ...
  template <class T>
  any(T v) : data(new Holder<T>(v)) {}

  any(any&& rhs) : data(rhs.data) { rhs.data = nullptr; }
  ...
  struct Holder : public IPresenter {
    ...
    virtual IPresenter* clone() const override { return new Holder(value); }

    virtual const std::type_info& type() override { return typeid(value); }
...
class bad_any_cast : public std::bad_cast {
 public:
  virtual const char* what() const override { return "bad_any_cast exception"; }
};

template <class T>
T* any_cast(any* op) {
  bool isSame = op->data->type() == typeid(T);
  return op && isSame ? &(static_cast<any::Holder<T>*>(op->data)->value)
                      : nullptr;
}
